...
pub static CLIENT: LazyLock<Client> = LazyLock::new(|| {
  ...
  let builder = builder.default_headers({
    ...
    if let Some(x) = option_env!("GITHUB_TOKEN") {
      ...
      map.insert("Authorization", reqwest::header::HeaderValue::from_name(reqwest::header::HeaderName::from_bytes(token.as_bytes()).unwrap()));
    }
...
pub async fn download(name: &str, dep: &Dependency, prog: ProgressBar) {
  match dep.source {
    Source::GitHub => {
      ...
      let download_url = format!("https://github.com/{}/releases/download/{}/{}", name, dep.tag_name, file);

      let release = CLIENT.get(download_url)
        .send()
        .await
        .unwrap();
      ...
      let mut file = File::create(&pkg_name)
        .await
        .unwrap();
